...
export async function buildFinalMessages(
  ...
  const processedChatMessages = chatMessages.map((chatMessage, index) => {
    ...
    if (nextChatMessageFileItems.length > 0) {
      ...
      console.log("retrievalText", retrievalText)
...
function buildRetrievalText(fileItems: Tables<"file_items">[]) {
  ...
  return `<||pdf_context||>You may use the following sources if needed to answer the user's question. If you don't know the answer, say "I don't know."\n\n${retrievalText}`
}
